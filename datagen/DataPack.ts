import { copy, dirname, exists } from "./deps.ts";
...
declare namespace DataPack {
  ...
  export type Recipe = Minecraft.Recipe | FarmersDelight.Recipe;
...
class DataPack {
  ...
  async writeRecipeFile(
    ...
    await Deno.mkdir(dirname(path), { recursive: true });
    await writeJsonFile(path, recipe);
  }
